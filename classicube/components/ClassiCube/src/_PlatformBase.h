...
void* TempMem_Alloc(int size) {
	if (!temp_mem) temp_mem = (int*)rg_alloc(65536, MEM_SLOW);
	if (size > 65536) Process_Abort("TempMem overflow");
...
#ifdef CC_BUILD_RETROGO
	return size ? rg_alloc(size, MEM_SLOW) : NULL;
#else
...
#ifdef CC_BUILD_RETROGO
	cc_uint32 size = CalcMemSize(numElems, elemsSize);
	void* ptr = size ? rg_alloc(size, MEM_SLOW) : NULL;
...
*#########################################################################################################################*/
#ifdef CC_BUILD_RETROGO
static char (*gameArgs)[STRING_SIZE];
#else
...
static CC_INLINE cc_result SetGameArgs(const cc_string* args, int numArgs) {
	int i;
#ifdef CC_BUILD_RETROGO
	if (!gameArgs) gameArgs = (char (*)[STRING_SIZE])rg_alloc(GAME_MAX_CMDARGS * STRING_SIZE, MEM_SLOW);
#endif
	for (i = 0; i < numArgs; i++) 
	{
