...
#else
static CC_BIG_VAR BlockID Builder_Chunk[EXTCHUNK_SIZE_3];
static CC_BIG_VAR cc_uint8 Builder_Counts[CHUNK_SIZE_3 * FACE_COUNT];
static CC_BIG_VAR int Builder_BitFlags[EXTCHUNK_SIZE_3];
#endif
...
#ifdef CC_BUILD_RETROGO
static struct Builder1DPart* Builder_Parts;
#else
...
static cc_bool ReadChunkData(int x1, int y1, int z1, cc_bool* outAllAir) {
	BlockRaw* blocks = World.MapBlocks;
...
#ifdef CC_BUILD_RETROGO
	Mem_Set(Builder_Parts, 0, sizeof(struct Builder1DPart) * ATLAS1D_MAX_ATLASES * 2);
#else
