...
#ifdef CC_BUILD_RETROGO
	buffer->textBuffer     = (char*)rg_alloc(STRINGSBUFFER_BUFFER_DEF_SIZE, MEM_SLOW);
	buffer->flagsBuffer    = (cc_uint32*)rg_alloc(STRINGSBUFFER_FLAGS_DEF_ELEMS * 4, MEM_SLOW);
...
#ifdef CC_BUILD_RETROGO
	if (buffer->textBuffer) Mem_Free(buffer->textBuffer);
	if (buffer->flagsBuffer) Mem_Free(buffer->flagsBuffer);
#else
