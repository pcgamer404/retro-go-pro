...
static void Physics_Activate(int index) {
	BlockID block = World.MapBlocks[index];
...
static void Physics_HandleSapling(int index, BlockID block) {
	...
	below = BLOCK_AIR;
	if (y > 0) below = World.MapBlocks[index - World.OneY];
...
static void Physics_PropagateLava(int posIndex, int x, int y, int z) {
	BlockID block = World.MapBlocks[posIndex];
...
static void Physics_HandleSlab(int index, BlockID block) {
	...
	if (World.MapBlocks[index - World.OneY] != BLOCK_SLAB) return;
	Game_UpdateBlock(x, y,     z, BLOCK_AIR);
...
static void Physics_HandleCobblestoneSlab(int index, BlockID block) {
	...
	if (World.MapBlocks[index - World.OneY] != BLOCK_COBBLE_SLAB) return;
	Game_UpdateBlock(x, y,     z, BLOCK_AIR);
...
#ifdef CC_BUILD_RETROGO
	if (!Physics_Ptr) Physics_Ptr = (struct_Physics*)rg_alloc(sizeof(struct_Physics), MEM_SLOW);
#endif
...
void Physics_Tick(void) {
	if (!Physics.Enabled || !World.MapBlocks) return;
