...
#include <rg_system.h>
static float *L_soupHeat, *L_potHeat, *L_flameHeat;
#else
...
*#########################################################################################################################*/
#ifdef CC_BUILD_RETROGO
static float *W_soupHeat, *W_potHeat, *W_flameHeat;
#else
...
#ifdef CC_BUILD_RETROGO
static struct AnimationData* anims_list;
#else
...
static void Animations_ReadDescription(struct Stream* stream, const cc_string* path) {
	...
	for (;;) {
		...
		if (anims_count == ATLAS1D_MAX_ATLASES) {
...
static void Animations_Apply(struct AnimationData* data) {
	...
	if (data->statesCount <= 0 || !anims_bmp.scan0) return;

	data->state++;
	if (data->state >= data->statesCount) data->state = 0;
...
static struct TextureEntry animations_entry = { "animations.png", AnimationsPngProcess, NULL };
static struct TextureEntry animations_txt   = { "animations.txt", Animations_ReadDescription, NULL };

static void UseWaterProcess(struct Stream* stream, const cc_string* name) {
	useWaterAnim    = true;
	alwaysWaterAnim = true;
}
static struct TextureEntry water_entry = { "usewateranim", UseWaterProcess, NULL };

static void UseLavaProcess(struct Stream* stream, const cc_string* name) {
	useLavaAnim    = true;
	alwaysLavaAnim = true;
}
static struct TextureEntry lava_entry = { "uselavaanim", UseLavaProcess, NULL };
...
#ifdef CC_BUILD_RETROGO
	if (!anims_list) {
		L_soupHeat = (float*)rg_alloc(LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float), MEM_SLOW);
		L_potHeat = (float*)rg_alloc(LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float), MEM_SLOW);
		L_flameHeat = (float*)rg_alloc(LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float), MEM_SLOW);
		W_soupHeat = (float*)rg_alloc(LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float), MEM_SLOW);
		W_potHeat = (float*)rg_alloc(LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float), MEM_SLOW);
		W_flameHeat = (float*)rg_alloc(LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float), MEM_SLOW);
		Mem_Set(L_soupHeat, 0, LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float));
		Mem_Set(L_potHeat, 0, LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float));
		Mem_Set(L_flameHeat, 0, LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float));
		Mem_Set(W_soupHeat, 0, LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float));
		Mem_Set(W_potHeat, 0, LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float));
		Mem_Set(W_flameHeat, 0, LIQUID_ANIM_MAX * LIQUID_ANIM_MAX * sizeof(float));
		anims_list = (struct AnimationData*)rg_alloc(ATLAS1D_MAX_ATLASES * sizeof(struct AnimationData), MEM_SLOW);
		Mem_Set(anims_list, 0, ATLAS1D_MAX_ATLASES * sizeof(struct AnimationData));
		anims_count = 0;
	}
#endif
