...
struct _TabListData* TabList_Ptr;
#else
...
#ifdef CC_BUILD_RETROGO
	if (!TabList_Ptr) {
		TabList_Ptr = (struct _TabListData*)rg_alloc(sizeof(struct _TabListData), MEM_SLOW);
		Mem_Set(TabList_Ptr, 0, sizeof(struct _TabListData));
		StringsBuffer_Init(&TabList._buffer);
	}
#endif
...
*#########################################################################################################################*/
#ifdef CC_BUILD_RETROGO
struct LocalPlayer* LocalPlayer_Instances;
#else
...
*#########################################################################################################################*/
#ifdef CC_BUILD_RETROGO
struct NetPlayer* NetPlayers_List;
#else
...
static void Entities_Init(void) {
	int i;
#ifdef CC_BUILD_RETROGO
	NetPlayers_List = (struct NetPlayer*)rg_alloc(MAX_NET_PLAYERS * sizeof(struct NetPlayer), MEM_SLOW);
	LocalPlayer_Instances = (struct LocalPlayer*)rg_alloc(MAX_LOCAL_PLAYERS * sizeof(struct LocalPlayer), MEM_SLOW);
#endif
