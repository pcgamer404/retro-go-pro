...
void World_Reset(void) {
	...
	if (World.MapBlocks != World.MapBlocks2) Mem_Free(World.MapBlocks2);
	World.MapBlocks2 = NULL;
	World.IDMask  = 0xFF;
#endif
	Mem_Free(World.MapBlocks);
	World.MapBlocks = NULL;
...
void World_SetNewMap(BlockRaw* blocks, int width, int height, int length) {
	...
	World.MapBlocks      = blocks;
	World.Name.length = 0;

	if (!World.Volume) World.MapBlocks = NULL;
#ifdef EXTENDED_BLOCKS
	/* .cw maps may have set this to a non-NULL when importing */
	if (!World.MapBlocks2) {
		World.MapBlocks2 = World.MapBlocks;
		World.IDMask  = 0xFF;
	}
#endif
...
#else
void World_SetBlock(int x, int y, int z, BlockID block) {
	World.MapBlocks[World_Pack(x, y, z)] = block; 
}
#endif
