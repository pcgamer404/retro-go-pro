...
#include <rg_system.h>
static struct SelectionBox* selections_list;
static cc_uint8* selections_ids;
#else
...
#ifdef CC_BUILD_RETROGO
	selections_list = (struct SelectionBox*)rg_alloc(SELECTIONS_MAX * sizeof(struct SelectionBox), MEM_SLOW);
	selections_ids  = (cc_uint8*)rg_alloc(SELECTIONS_MAX * sizeof(cc_uint8), MEM_SLOW);
#endif
