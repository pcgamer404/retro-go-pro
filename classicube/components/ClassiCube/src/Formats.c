...
static cc_result Map_ReadBlocks(struct Stream* stream) {
	...
	World.MapBlocks = (BlockRaw*)Mem_TryAlloc(World.Volume, 1);

	if (!World.MapBlocks) return ERR_OUT_OF_MEMORY;
	return Stream_Read(stream, World.MapBlocks, World.Volume);
}
...
cc_result Map_LoadFrom(const cc_string* path) {
	...
	World_SetNewMap(World.MapBlocks, World.Width, World.Height, World.Length);
			...
			for (x = 0; x < World.Width; x += LVL_CHUNKSIZE) {
				...
				if ((x + LVL_CHUNKSIZE) <= adjWidth && (y + LVL_CHUNKSIZE) <= adjHeight && (z + LVL_CHUNKSIZE) <= adjLength) {
					for (i = 0; i < sizeof(chunk); i++) {
						...
						World.MapBlocks[index] = World.MapBlocks[index] == LVL_CUSTOMTILE ? chunk[i] : World.MapBlocks[index];
					}
				} else {
					for (i = 0; i < sizeof(chunk); i++) {
						xx = i & 0xF; yy = (i >> 8) & 0xF; zz = (i >> 4) & 0xF;
...
static void Cw_Callback_1(struct NbtTag* tag) {
	...
	if (IsTag(tag, "BlockArray")) {
		...
		World.MapBlocks = Nbt_TakeArray(tag, ".cw map blocks");
	}
...
static cc_result Dat_LoadFormat0(struct Stream* stream) {
	...
	World.MapBlocks = (BlockRaw*)Mem_TryAlloc(PC_VOLUME, 1);
	if (!World.MapBlocks) return ERR_OUT_OF_MEMORY;

	/* First 5 bytes already read earlier as .dat header */
	Mem_Set(World.MapBlocks, BLOCK_STONE, 5);
	return Stream_Read(stream, World.MapBlocks + 5, PC_VOLUME - 5);
}
...
static cc_result Dat_LoadFormat2(struct Stream* stream) {
	...
	for (i = 0; i < desc->FieldsCount; i++) 
	{
		...
		if (String_CaselessEqualsConst(&fieldName, "width")) {
			...
			World.MapBlocks = field->Value.Array.Ptr;
...
static void MCLevel_ParseMap(struct NbtTag* tag) {
	...
	if (IsTag(tag, "blocks")) {
		World.Volume = tag->dataSize;
		World.MapBlocks = Nbt_TakeArray(tag, ".mclevel map blocks");
	}
}
...
cc_result Cw_Save(struct Stream* stream) {
	...
	if ((res = Stream_Write(stream, World.MapBlocks, World.Volume)))  return res;

#ifdef EXTENDED_BLOCKS
	if (World.MapBlocks != World.MapBlocks2) {
		cur = buffer;
		cur = Nbt_WriteArray(cur, "BlockArray2", World.Volume);

		if ((res = Stream_Write(stream, buffer, (int)(cur - buffer)))) return res;
		if ((res = Stream_Write(stream, World.MapBlocks2, World.Volume))) return res;
	}
#endif
...
static struct JField {
	...
	{ JFIELD_I32, false, "width",  NULL },
	{ JFIELD_I32, false, "depth",  NULL },
	{ JFIELD_I32, false, "height", NULL },
	{ JFIELD_I32, true,  "xSpawn", NULL },
	{ JFIELD_I32, true,  "ySpawn", NULL },
	{ JFIELD_I32, true,  "zSpawn", NULL },
	{ JFIELD_ARRAY,0, "blocks", NULL }
...
cc_result Dat_Save(struct Stream* stream) {
	...
	level_fields[0].value = &World.Width;
	level_fields[1].value = &World.Height;
	level_fields[2].value = &World.Length;
	level_fields[3].value = &LocalPlayer_Instances[0].Base.Position.x;
	level_fields[4].value = &LocalPlayer_Instances[0].Base.Position.y;
	level_fields[5].value = &LocalPlayer_Instances[0].Base.Position.z;
	level_fields[6].value = NULL; /* blocks handled separately */
...
*-------------------------------------------------------Formats component-------------------------------------------------*
*#########################################################################################################################*/
static struct MapImporter cw_imp    = { ".cw",      Cw_Load, NULL };
static struct MapImporter dat_imp   = { ".dat",     Dat_Load, NULL };
static struct MapImporter lvl_imp   = { ".lvl",     Lvl_Load, NULL };
static struct MapImporter mine_imp  = { ".mine",    Dat_Load, NULL };
static struct MapImporter fcm_imp   = { ".fcm",     Fcm_Load, NULL };
static struct MapImporter mclvl_imp = { ".mclevel", MCLevel_Load, NULL };
