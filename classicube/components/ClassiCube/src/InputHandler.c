...
#endif
enum MouseButton_ { MOUSE_LEFT, MOUSE_RIGHT, MOUSE_MIDDLE, MOUSE_MAX };
...
static struct LocalPlayerInput gamepadInput = { PlayerInputGamepad, NULL };
...
struct HotkeyData* HotkeysList;
struct StringsBuffer* HotkeysText_Ptr;
#else
...
static struct LocalPlayerInput normalInput = { PlayerInputNormal, NULL };

static void OnInit(void) {
#ifdef CC_BUILD_RETROGO
	HotkeysList = (struct HotkeyData*)rg_alloc(sizeof(struct HotkeyData) * HOTKEYS_MAX_COUNT, MEM_SLOW);
	HotkeysText_Ptr = (struct StringsBuffer*)rg_alloc(sizeof(struct StringsBuffer), MEM_SLOW);
	StringsBuffer_Init(HotkeysText_Ptr);
#endif
