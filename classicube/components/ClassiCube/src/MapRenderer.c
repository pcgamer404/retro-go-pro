...
#include <rg_system.h>
static int *normPartsCount, *tranPartsCount;
static cc_bool *hasNormParts, *hasTranParts;
static cc_bool *checkNormParts, *checkTranParts;
#else
...
static void RefreshBorderChunks(int maxHeight) {
	...
	chunkPos = IVec3_MaxValue();
	if (!mapChunks || !World.MapBlocks) return;
...
#ifdef CC_BUILD_RETROGO
	normPartsCount = (int*)rg_alloc(ATLAS1D_MAX_ATLASES * sizeof(int), MEM_SLOW);
	tranPartsCount = (int*)rg_alloc(ATLAS1D_MAX_ATLASES * sizeof(int), MEM_SLOW);
	hasNormParts = (cc_bool*)rg_alloc(ATLAS1D_MAX_ATLASES * sizeof(cc_bool), MEM_SLOW);
	hasTranParts = (cc_bool*)rg_alloc(ATLAS1D_MAX_ATLASES * sizeof(cc_bool), MEM_SLOW);
	checkNormParts = (cc_bool*)rg_alloc(ATLAS1D_MAX_ATLASES * sizeof(cc_bool), MEM_SLOW);
	checkTranParts = (cc_bool*)rg_alloc(ATLAS1D_MAX_ATLASES * sizeof(cc_bool), MEM_SLOW);
	ResetPartCounts();
#endif
