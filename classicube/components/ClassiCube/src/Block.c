...
static const struct SimpleBlockDef invalid_blockDef = { 
	"Invalid", 0,0,0,16, FOG_NONE,0, false,true, 100, DRAW_OPAQUE,COLLIDE_SOLID,0,0
};
...
#ifdef CC_BUILD_RETROGO
	Blocks_Ptr = (struct _BlockLists*)rg_alloc(sizeof(struct _BlockLists), MEM_SLOW);
	Block_NamesBuffer = (char*)rg_alloc(STRING_SIZE * BLOCK_COUNT, MEM_SLOW);
#endif
