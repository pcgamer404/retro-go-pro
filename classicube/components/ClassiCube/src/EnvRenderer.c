...
static int CalcRainHeightAt(int x, int maxY, int z, int hIndex) {
	...
	RainCalcBody(World.MapBlocks[i]);
#else
	if (World.IDMask <= 0xFF) {
		RainCalcBody(World.MapBlocks[i]);
	} else {
		RainCalcBody(World.MapBlocks[i] | (World.MapBlocks2[i] << 8));
	}
#endif
...
static struct TextureEntry clouds_entry = { "clouds.png", CloudsPngProcess, NULL };
