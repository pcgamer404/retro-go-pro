...
static void NotchyGen_CreateHeightmap(void) {
	...
	/* Only initialize noises once at the beginning of the generation! */
	if (current_z == 0 && hIndex == 0) {
	...
	for (; current_z < World.Length; current_z++) {
		Gen_CurrentProgress = (float)current_z / World.Length;

		for (x = 0; x < World.Width; x++) {
			hLow   = CombinedNoise_Calc(n1, x * 1.3f, current_z * 1.3f) / 6 - 4;
			height = hLow;

			if (OctaveNoise_Calc(n3, (float)x, (float)current_z) <= 0) {
				hHigh = CombinedNoise_Calc(n2, x * 1.3f, current_z * 1.3f) / 5 + 6;
		...
		/* Cooperative yield check inside the loop */
		#ifdef CC_BUILD_COOPTHREADED
		cc_uint64 curTime = Stopwatch_Measure();
		if (Stopwatch_ElapsedMS(lastRender, curTime) > 100) {
			lastRender = curTime;
			current_z++; /* Advance for next time */
			return; /* Yield back */
		}
		#endif
	}

	gen_step++; /* Advance to next state */
	current_z = 0;
	hIndex = 0;
}
...
static cc_bool NotchyGen_Prepare(int seed) {
	...
	heightmap  = (cc_int16*)rg_alloc(World.Width * World.Length * 2, MEM_SLOW);
