...
#ifdef CC_BUILD_RETROGO
#include <rg_system.h>
static struct StringsBuffer* ccCookies_Ptr;
#define ccCookies (*ccCookies_Ptr)
#else
#ifdef CC_BUILD_RETROGO
static struct StringsBuffer* ccCookies_Ptr;
#define ccCookies (*ccCookies_Ptr)
...
#ifdef CC_BUILD_RETROGO
	if (!ccCookies_Ptr) {
		ccCookies_Ptr = (struct StringsBuffer*)rg_alloc(sizeof(struct StringsBuffer), MEM_SLOW);
		StringsBuffer_Init(ccCookies_Ptr);
	}
#endif
