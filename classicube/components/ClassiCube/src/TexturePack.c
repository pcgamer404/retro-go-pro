...
#ifdef CC_BUILD_RETROGO
struct _Atlas1DData* Atlas1D_Ptr;
#else
...
#ifdef CC_BUILD_RETROGO
static struct StringsBuffer *acceptedList_Ptr, *deniedList_Ptr;
#define acceptedList (*acceptedList_Ptr)
#define deniedList   (*deniedList_Ptr)
#else
...
#ifdef CC_BUILD_RETROGO
	if (!acceptedList_Ptr) {
		acceptedList_Ptr = (struct StringsBuffer*)rg_alloc(sizeof(struct StringsBuffer), MEM_SLOW);
		deniedList_Ptr   = (struct StringsBuffer*)rg_alloc(sizeof(struct StringsBuffer), MEM_SLOW);
		StringsBuffer_Init(acceptedList_Ptr);
		StringsBuffer_Init(deniedList_Ptr);
	}
#endif
...
#ifdef CC_BUILD_NETWORKING
#ifdef CC_BUILD_RETROGO
static struct StringsBuffer *etagCache_Ptr, *lastModCache_Ptr;
#define etagCache    (*etagCache_Ptr)
#define lastModCache (*lastModCache_Ptr)
#else
...
#ifdef CC_BUILD_RETROGO
	if (!etagCache_Ptr) {
		etagCache_Ptr    = (struct StringsBuffer*)rg_alloc(sizeof(struct StringsBuffer), MEM_SLOW);
		lastModCache_Ptr = (struct StringsBuffer*)rg_alloc(sizeof(struct StringsBuffer), MEM_SLOW);
		StringsBuffer_Init(etagCache_Ptr);
		StringsBuffer_Init(lastModCache_Ptr);
	}
#endif
...
static struct TextureEntry terrain_entry = { "terrain.png", TerrainPngProcess, NULL };
...
#ifdef CC_BUILD_RETROGO
	if (!Atlas1D_Ptr) {
		Atlas1D_Ptr = (struct _Atlas1DData*)rg_alloc(sizeof(struct _Atlas1DData), MEM_SLOW);
		Mem_Set(Atlas1D_Ptr, 0, sizeof(struct _Atlas1DData));
	}
#endif
