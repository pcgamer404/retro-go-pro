...
#ifdef CC_BUILD_RETROGO
struct StringsBuffer* Options_Ptr;
struct StringsBuffer* changedOpts_Ptr;
#define changedOpts (*changedOpts_Ptr)
#else
...
#ifdef CC_BUILD_RETROGO
	Options_Ptr = (struct StringsBuffer*)rg_alloc(sizeof(struct StringsBuffer), MEM_SLOW);
	changedOpts_Ptr = (struct StringsBuffer*)rg_alloc(sizeof(struct StringsBuffer), MEM_SLOW);
	StringsBuffer_Init(Options_Ptr);
	StringsBuffer_Init(changedOpts_Ptr);
#endif
...
#ifdef CC_BUILD_RETROGO
	if (!Options_Ptr) { *value = String_Empty; return false; }
#endif
