...
static int ClassicLighting_CalcHeightAt(int x, int maxY, int z, int hIndex) {
	...
	ClassicLighting_CalcBody(World.MapBlocks[i]);
#else
	if (World.IDMask <= 0xFF) {
		ClassicLighting_CalcBody(World.MapBlocks[i]);
	} else {
		ClassicLighting_CalcBody(World.MapBlocks[i] | (World.MapBlocks2[i] << 8));
	}
#endif
...
#ifndef EXTENDED_BLOCKS
	ClassicLighting_NeedsNeighourBody(World.MapBlocks[i]);
#else
	if (World.IDMask <= 0xFF) {
		ClassicLighting_NeedsNeighourBody(World.MapBlocks[i]);
	} else {
		ClassicLighting_NeedsNeighourBody(World.MapBlocks[i] | (World.MapBlocks2[i] << 8));
	}
#endif
	return false;
}
...
#ifndef EXTENDED_BLOCKS
	Heightmap_CalculateBody(World.MapBlocks[mapIndex]);
#else
	if (World.IDMask <= 0xFF) {
		Heightmap_CalculateBody(World.MapBlocks[mapIndex]);
	} else {
		Heightmap_CalculateBody(World.MapBlocks[mapIndex] | (World.MapBlocks2[mapIndex] << 8));
	}
#endif
	return false;
}
