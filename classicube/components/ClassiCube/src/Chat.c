...
#ifdef CC_BUILD_RETROGO
struct StringsBuffer* Chat_Log_Ptr;
struct StringsBuffer* Chat_InputLog_Ptr;
#else
...
void Chat_AddOf(const cc_string* text, int msgType) {
	...
	if (!Chat_Log_Ptr) {
		Platform_Log("CHAT: ", 6);
		Platform_Log(text->buffer, text->length);
		return;
	}
#endif
...
#ifdef CC_BUILD_RETROGO
	Chat_Log_Ptr = (struct StringsBuffer*)rg_alloc(sizeof(struct StringsBuffer), MEM_SLOW);
	Chat_InputLog_Ptr = (struct StringsBuffer*)rg_alloc(sizeof(struct StringsBuffer), MEM_SLOW);
	StringsBuffer_Init(Chat_Log_Ptr);
	StringsBuffer_Init(Chat_InputLog_Ptr);
#endif
