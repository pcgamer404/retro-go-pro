...
#ifdef EXTENDED_BLOCKS
	/* Masks access to World.MapBlocks/World.MapBlocks2 */
	/* e.g. this will be 255 if only 8 bit blocks are used */
	int IDMask;
#endif
	/* Whether the world has finished loading/generating. */
	/* NOTE: MapBlocks may still be NULL. (e.g. error during loading) */
	cc_bool Loaded;
...
#ifdef EXTENDED_BLOCKS
/* Sets World.MapBlocks2 and updates internal state for more than 256 blocks. */
void World_SetMapUpper(BlockRaw* blocks);

#define World_GetRawBlock(idx) ((World.MapBlocks[idx] | (World.MapBlocks2[idx] << 8)) & World.IDMask)
...
#else
#define World_GetBlock(x, y, z) World.MapBlocks[World_Pack(x, y, z)]
#define World_GetRawBlock(idx)  World.MapBlocks[idx]
#endif
...
#define ENV_DEFAULT_FOG_COLOR       PackedCol_Make(0x99, 0xCC, 0xFF, 0xFF)
