...
#undef Physics
#define Physics_Instance (*Physics_Ptr)
#else
#define Physics_Instance Physics
#endif
#include "Widgets.h"
#include "Game.h"
#include "Event.h"
...
static void HS_SetJump(const cc_string* v) {
	...
	struct PhysicsComp* pComp;

	pComp = &Entities.CurPlayer->Physics;
	pComp->JumpVel     = PhysicsComp_CalcJumpVelocity(Menu_Float(v));
	pComp->UserJumpVel = pComp->JumpVel;

	String_InitArray(str, strBuffer);
	String_AppendFloat(&str, pComp->JumpVel, 8);
...
static cc_bool MiO_GetPhysics(void) { return Physics_Instance.Enabled; }
...
static void MiscSettingsScreen_InitWidgets(struct MenuOptionsScreen* s) {
	MenuOptionsScreen_BeginButtons(s);
	{
		...
		MenuOptionsScreen_AddBool(s, "Block pComp",
