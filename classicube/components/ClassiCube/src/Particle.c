...
#include <rg_system.h>
static struct Particle* rain_Particles;
#else
...
#ifdef CC_BUILD_RETROGO
static struct TerrainParticle* terrain_particles;
#else
...
#ifdef CC_BUILD_RETROGO
static struct CustomParticle* custom_particles;
#else
...
static struct TextureEntry particles_entry = { "particles.png", ParticlesPngProcess, NULL };
...
#ifdef CC_BUILD_RETROGO
	rain_Particles    = (struct Particle*)rg_alloc(PARTICLES_MAX * sizeof(struct Particle), MEM_SLOW);
	terrain_particles = (struct TerrainParticle*)rg_alloc(PARTICLES_MAX * sizeof(struct TerrainParticle), MEM_SLOW);
#ifdef CC_BUILD_NETWORKING
	custom_particles  = (struct CustomParticle*)rg_alloc(PARTICLES_MAX * sizeof(struct CustomParticle), MEM_SLOW);
#endif
#endif
