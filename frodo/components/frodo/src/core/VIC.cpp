...
// Desktop Frodo trades 512 KiB of RAM for fast character-pixel expansion.
// That is larger than the ESP32's usable internal DRAM. Store only the two
// bit masks for each character byte and combine them with repeated palette
// bytes at draw time (2 KiB + 64 bytes).
static uint32_t TextBitMask[256][2];
static uint32_t RepeatedColor[16];

static inline uint32_t text_color(uint8_t foreground, uint8_t background,
	uint8_t data, unsigned half)
{
	const uint32_t mask = TextBitMask[data][half];
	return (mask & RepeatedColor[foreground & 0x0f]) |
		(~mask & RepeatedColor[background & 0x0f]);
}
...
{
	for (int i = 0; i < 16; ++i) {
		RepeatedColor[i] = uint32_t(colors[i]) * 0x01010101u;
	}
	for (int data = 0; data < 256; ++data) {
		uint8_t *left = reinterpret_cast<uint8_t *>(&TextBitMask[data][0]);
		uint8_t *right = reinterpret_cast<uint8_t *>(&TextBitMask[data][1]);
		for (int pixel = 0; pixel < 4; ++pixel) {
			left[pixel] = (data & (0x80 >> pixel)) ? 0xff : 0x00;
			right[pixel] = (data & (0x08 >> pixel)) ? 0xff : 0x00;
		}
	}
}
...
unsigned MOS6569::EmulateLine(int & retCyclesLeft)
{
	...
	if (raster == TOTAL_RASTERS) {
		...
		chunky_line_start = render_enabled ? the_display->BitmapBase()
		                                  : skipped_frame_line;
		xmod = render_enabled ? the_display->BitmapXMod() : 0;
	}
