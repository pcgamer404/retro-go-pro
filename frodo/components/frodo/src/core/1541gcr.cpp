...
static uint8_t *allocate_gcr_track(size_t size)
{
#ifdef ESP_PLATFORM
...
static void release_gcr_track(uint8_t *memory)
{
#ifdef ESP_PLATFORM
...
// Convert an optional D64 per-sector job status byte to Frodo's internal
// DOS error index. The desktop build gets this helper from ImageDrive; the
// embedded build deliberately omits that DOS-level drive implementation.
static int d64_error_info(uint8_t error)
{
	static constexpr int table[16] = {
		ERR_OK, ERR_OK, ERR_READ20, ERR_READ21,
		ERR_READ22, ERR_READ23, ERR_READ24, ERR_WRITE25,
		ERR_WRITEPROTECT, ERR_READ27, ERR_WRITE28, ERR_DISKID,
		ERR_OK, ERR_OK, ERR_OK, ERR_NOTREADY
	};
	return table[error & 0x0f];
}
...
bool GCRDisk::FlushImage()
{
	return the_file == nullptr || fflush(the_file) == 0;
}

void GCRDisk::ReloadImage(const std::string &filepath)
{
	close_image_file();
	open_image_file(filepath);
}
...
bool GCRDisk::load_image_file()
{
	...
	for (unsigned track = 1; track <= num_tracks; ++track) {
		...
		gcr_data[halftrack] = allocate_gcr_track(gcr_track_length[halftrack]);
		if (gcr_data[halftrack] == nullptr) {
			for (unsigned i = 0; i < MAX_NUM_HALFTRACKS; ++i) {
				release_gcr_track(gcr_data[i]);
				gcr_data[i] = nullptr;
				gcr_track_length[i] = 0;
...
bool GCRDisk::load_gcr_file()
{
	...
	for (unsigned halftrack = 0; halftrack < num_halftracks; ++halftrack) {
		...
		gcr_data[halftrack] = allocate_gcr_track(length);
		if (gcr_data[halftrack] == nullptr) {
			for (unsigned i = 0; i < MAX_NUM_HALFTRACKS; ++i) {
				release_gcr_track(gcr_data[i]);
				gcr_data[i] = nullptr;
...
bool GCRDisk::write_sector(unsigned track, unsigned sector, const uint8_t *buffer)
{
	...
	fseek(the_file, offset + header_size, SEEK_SET);
	return fwrite(buffer, 1, 256, the_file) == 256 && fflush(the_file) == 0;
}
