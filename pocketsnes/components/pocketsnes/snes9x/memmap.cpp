...
/**********************************************************************************************/
bool8 CMemory::Init (uint32 minimum_rom_size)
{
    // DS2 DMA notes: These would do well to be allocated with 32 extra bytes
    // so they can be 32-byte aligned. [Neb]
    RAM	    = (uint8 *) rg_alloc (0x20000, MEM_SLOW);
    SRAM    = (uint8 *) rg_alloc (0x20000, MEM_SLOW);
    VRAM    = (uint8 *) rg_alloc (0x10000, MEM_SLOW);
    C4RAM   = (uint8 *) rg_alloc (0x2000, MEM_SLOW);
    BSRAM   = NULL;

	FillRAM = NULL;

    // Reserve only enough contiguous ROM storage for the selected image. The
    // old largest-first probing consumed all spare PSRAM even for small games.
    const size_t AllocSizes[] = {0x80000, 0x100000, 0x200000, 0x280000, 0x300000, 0x400000, 0x600000, 0x800000, 0};
    for (const size_t *size = AllocSizes; *size && !ROM; ++size)
    {
        // A copier header may add 0x200 bytes and is stripped during loading.
        if (minimum_rom_size > *size + 0x200)
            continue;

        ROM_AllocSize = *size + 0x200 + 0x8000;
        ROM = (uint8 *) rg_alloc (ROM_AllocSize, MEM_SLOW | MEM_NOPANIC);
        break;
    }

    IPPU.TileCache [TILE_2BIT] = (uint8 *) rg_alloc (MAX_2BIT_TILES * 128, MEM_SLOW);
    IPPU.TileCache [TILE_4BIT] = (uint8 *) rg_alloc (MAX_4BIT_TILES * 128, MEM_SLOW);
    IPPU.TileCache [TILE_8BIT] = (uint8 *) rg_alloc (MAX_8BIT_TILES * 128, MEM_SLOW);
    
    IPPU.TileCached [TILE_2BIT] = (uint8 *) rg_alloc (MAX_2BIT_TILES, MEM_FAST);
    IPPU.TileCached [TILE_4BIT] = (uint8 *) rg_alloc (MAX_4BIT_TILES, MEM_FAST);
    IPPU.TileCached [TILE_8BIT] = (uint8 *) rg_alloc (MAX_8BIT_TILES, MEM_FAST);

	memset (RAM, 0, 0x20000);
	memset (SRAM, 0, 0x20000);
	memset (VRAM, 0, 0x10000);
    
    if (!RAM || !SRAM || !VRAM || !ROM || !C4RAM ||
...
again:
    ...
    if (filename == NULL)
    {
        TotalFileSize = ROM_FileSize;
        HeaderCount = 0;
    }
    else
    {
	    TotalFileSize = FileLoader(ROM, filename, ROM_AllocSize - 0x200 - 0x8000);
    }

	if (!TotalFileSize)
		return FALSE;		// it ends here
	else if(filename && !Settings.NoPatch)
    ...
    size_t max_rom_size = ROM_AllocSize - 0x200 - 0x8000;
    RG_LOGI("ROM loaded: TotalFileSize=%d, CalculatedSize=%d, MaxUsable=%d\n", 
            (int)TotalFileSize, (int)CalculatedSize, (int)max_rom_size);

    if (CalculatedSize < max_rom_size)
    {
    ...
    if (!Settings.ForceHiROM && !Settings.ForceLoROM &&
		...
		!Settings.ForceSA1Main && !Settings.ForceC4 &&
		!Settings.ForceSDD1)
    {
...
bool8 CMemory::SaveSRAM (const char *filename)
{
	if(Settings.SuperFX && Memory.ROMType < 0x15)
		return TRUE;
	if(Settings.SA1Main && Memory.ROMType == 0x34)
		return TRUE;
...
void CMemory::BSLoROMMap ()
{
    ...
    if (!BSRAM)
    {
        BSRAM = (uint8 *) rg_alloc (0x80000, MEM_SLOW | MEM_NOPANIC);
        if (!BSRAM) RG_PANIC("BS-X RAM allocation failed!");
...
void CMemory::SuperFXROMMap ()
{
    ...
    for (c = 0; c < 64; c++)
    {
        if (0x218000 + c * 0x10000 <= ROM_AllocSize)
        {
#ifdef DS2_DMA
...
const char *CMemory::KartContents ()
{
	...
	else if(Settings.SETA!=0)
	{
		...
		strcat (tmp, CoPro [(ROMType & 0xf0) >> 4]);
...
[14:25:27] <@Nach>     case 0x340f23e5: //Donkey Kong Country 3 (U) copier hack - handled
	*/
    ...
    if (strcmp (ROMId, "AVCJ") == 0 ||
		...
		strcmp (ROMName, "SFC ײް") == 0 || //Kamen Rider
    ...
    if (strcmp (ROMName, "̧߰н") == 0 ||  //Super Famista
		strcmp (ROMName, "̧߰н 2") == 0 || //Super Famista 2
...
//Apparently are needed!
    ...
    if (strcmp (ROMName, "SFX ŲĶɶ 1") == 0) //Gundam Knight Story
    {
