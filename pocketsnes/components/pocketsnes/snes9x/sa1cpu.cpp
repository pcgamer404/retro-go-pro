...
#if 0
    if (SA1_core.Flags & NMI_FLAG)
    {
	SA1_core.Flags &= ~NMI_FLAG;
	if (SA1_core.WaitingForInterrupt)
	{
	    SA1_core.WaitingForInterrupt = FALSE;
	    SA1_core.PC++;
	}
	S9xSA1Opcode_NMI ();
    }
#endif
    if (SA1_core.Flags & IRQ_PENDING_FLAG)
    {
	if (SA1_core.IRQActive)
	{
	    if (SA1_core.WaitingForInterrupt)
	    {
		SA1_core.WaitingForInterrupt = FALSE;
	    ...
	    SA1_core.Flags &= ~IRQ_PENDING_FLAG;
    }
#ifdef DEBUGGER
    if (SA1_core.Flags & TRACE_FLAG)
    {
	for (i = 0; i < 3 && SA1_core.Executing; i++)
	{
	    S9xSA1Trace ();
#ifdef CPU_SHUTDOWN
	    SA1_core.PCAtOpcodeStart = SA1_core.PC;
#endif
	    (*SA1_core.S9xOpcodes [*SA1_core.PC++].S9xOpcode) ();
	}
    }
    else
#endif
