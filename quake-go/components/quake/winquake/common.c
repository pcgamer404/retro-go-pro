...
void COM_CheckRegistered (void)
{
    ...
    Sys_Error ("This dedicated server requires a full registered copy of Quake");
        ...
        // Disable legacy check that prevents boot with non-original shareware PAKs
        // if (com_modified)
...
static int COM_FindFile_ESP32(char *filename, int *handle, QUAKE_FILE **file, const void **mmap)
{
    ...
    for (search = com_searchpaths; search; search = search->next)
    {
            ...
            for (i=0 ; i<pak->numfiles ; i++)
                ...
                {       // found it!
                    ...
                    else //mmap
                    {
                        ...
                        if (mmapBase == NULL)
                        {
                            Sys_Printf("File was found in pak but Sys_FileGetMmapBase returned NULL. Trying next search path element.\n");
                            continue;
                        }
        ...
        else // Not a pak file, so it's a direct filesystem entry
        {               
...
//              Con_Printf ("Couldn't open %s\n", packfile);
    ...
    info = heap_caps_malloc(header.dirlen, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (!info)
    {
        Sys_FileClose(packhandle);
        Sys_Error ("Couldn't allocate %d bytes for pack info", header.dirlen);
    }
