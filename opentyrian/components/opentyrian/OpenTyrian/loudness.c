...
static SemaphoreHandle_t music_mutex = NULL;
...
bool init_audio( void )
{
	...
	if (!music_mutex)
		music_mutex = xSemaphoreCreateMutex();
	
	for (unsigned int i = 0; i < SFX_CHANNELS; i++)
	{
		channel_cap[i] = DEFAULT_CHANNEL_CAP;
		channel_buffer[i] = (SAMPLE_TYPE *)rg_alloc(channel_cap[i], MEM_SLOW);
		channel_pos[i] = NULL;
		channel_len[i] = 0;
	}
...
IRAM_ATTR void audio_cb( void *user_data, unsigned char *sdl_buffer, int howmuch )
{
	...
	if (!samples_disabled)
	{
		...
		for (int ch = 0; ch < SFX_CHANNELS; ch++)
		{
			...
			/* SYN: If we've emptied a channel buffer, clear active pointer (retain buffer for reuse) */
			if (channel_len[ch] == 0)
			{
				channel_pos[ch] = NULL;
			}
...
{
	if (audio_disabled || samples_disabled || chan >= SFX_CHANNELS || !buffer || size == 0)
		return;
	
	Uint32 needed = (Uint32)size * BYTES_PER_SAMPLE * SAMPLE_SCALING;

	SDL_LockAudio();
	
	if (needed > channel_cap[chan])
	{
		if (channel_buffer[chan])
			free(channel_buffer[chan]);
		channel_buffer[chan] = (SAMPLE_TYPE *)rg_alloc(needed, MEM_SLOW);
		channel_cap[chan] = channel_buffer[chan] ? needed : 0;
	}

	if (!channel_buffer[chan])
	{
		channel_len[chan] = 0;
		channel_pos[chan] = NULL;
		SDL_UnlockAudio();
		return;
	}

	channel_len[chan] = needed;
