...
// per-chip variables
Bitu chip_num;
EXT_RAM_BSS_ATTR op_type op[MAXOPERATORS];
...
static EXT_RAM_BSS_ATTR Bit16s wavtable[WAVEPREC*3];	// wave form table

// vibrato/tremolo tables
static EXT_RAM_BSS_ATTR Bit32s vib_table[VIBTAB_SIZE];
static EXT_RAM_BSS_ATTR Bit32s trem_table[TREMTAB_SIZE*2];

static EXT_RAM_BSS_ATTR Bit32s vibval_const[BLOCKBUF_SIZE];
static EXT_RAM_BSS_ATTR Bit32s tremval_const[BLOCKBUF_SIZE];

// vibrato value tables (used per-operator)
static EXT_RAM_BSS_ATTR Bit32s vibval_var1[BLOCKBUF_SIZE];
static EXT_RAM_BSS_ATTR Bit32s vibval_var2[BLOCKBUF_SIZE];
...
OPL_INLINE static void operator_advance(op_type* op_pt, Bit32s vib) {
...
OPL_INLINE static void operator_advance_drums(op_type* op_pt1, Bit32s vib1, op_type* op_pt2, Bit32s vib2, op_type* op_pt3, Bit32s vib3) {
...
OPL_INLINE static void operator_output(op_type* op_pt, Bit32s modulator, Bit32s trem) {
	if (op_pt->op_state != OF_TYPE_OFF) {
		...
		op_pt->cval = (Bit32s)(op_pt->step_amp*op_pt->vol*op_pt->cur_wform[i&op_pt->cur_wmask]*trem*0.0625f);
	}
}
