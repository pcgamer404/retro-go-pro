...
struct PicoRam
{
    ...
    union
    {
        struct
        {
            // 0x0000 - 0x1fff: Sprite sheet (128x128 pixels = 8192 bytes)
            // Sprites 128-255 (0x1000 - 0x1fff) share memory with Map rows 32-63
            uint8_t spriteSheetData[8192];

            // 0x2000 - 0x2fff: Map rows 0-31 (4096 bytes)
            uint8_t mapData[4096];
