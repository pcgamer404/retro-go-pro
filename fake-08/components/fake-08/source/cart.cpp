...
bool Cart::loadCartFromPixels(const uint8_t* image, size_t imageBytes) {
...
bool Cart::loadCartFromPng(const uint8_t* data, size_t size){
    unsigned char* out = NULL;
    unsigned width, height;

    //decode
    unsigned error = lodepng_decode32(&out, &width, &height, (const unsigned char*)data, size);

    //if there's an error, display it
    if(error) {
        Logger_Write("lodepng error: %u\n", error);
        if (out) free(out);
        return false;
    }
    ...
    bool success = loadCartFromPixels(out, (size_t)width * height * 4);
    free(out);
    return success;
}
...
Cart::Cart (const unsigned char* cartData, size_t size){
    ...
    if ((char)cartData[0] == '\x89' && 
        ...
        (char)cartData[2] == 'N' && 
        (char)cartData[3] == 'G') {
        bool success = loadCartFromPng((const uint8_t*)cartData, size);
...
Cart::Cart(std::string filename, std::string cartDirectory){
    ...
    if (isDefaultCart || isSettingsCart || firstFourChars == "pico"){
        ...
        std::vector<char> buffer = get_file_as_buffer(FullCartPath);
        bool success = loadCartFromPng((const uint8_t*)buffer.data(), buffer.size());
...
void Cart::initCartRom(){
    ...
    for(size_t i = 0; i < 64; i++) {
        memset(&CartRom.SfxData[i], 0, sizeof(sfx));
    }
    for(size_t i = 0; i < 64; i++) {
