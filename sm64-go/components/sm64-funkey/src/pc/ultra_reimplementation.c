...
void osMapTLB(s32 index, OSPageMask pm, void *vaddr, u32 paddr0, u32 paddr1, s32 page_mask) {
}

u8 _gd_dynlistsSegmentRomStart[1] = {0};
u8 _gd_dynlistsSegmentRomEnd[1] = {0};
...
#else
    FILE *fp = fopen_home("sm64_save_file.bin", "rb");
    if (fp == NULL) {
        return -1;
    }
...
s32 osEepromLongWrite(UNUSED OSMesgQueue *mq, u8 address, u8 *buffer, int nbytes) {
    ...
    s32 ret = 0;
#else
    FILE* fp = fopen_home("sm64_save_file.bin", "wb");
    if (fp == NULL) {
        return -1;
    }
