...
static s32 intro_title_context_matrix_valid(Mat4 mtx) {
    return mtx != NULL
        && fabsf(mtx[3][3] - 1.0f) < 0.01f
        && fabsf(mtx[0][0]) + fabsf(mtx[1][1]) + fabsf(mtx[2][2]) > 0.01f;
}

static void intro_title_make_default_camera(Mat4 dest) {
    Vec3f pos = { 0.0f, 0.0f, 3200.0f };
    Vec3f focus = { 0.0f, 0.0f, 0.0f };

    mtxf_lookat(dest, pos, focus, 0);
}
...
Gfx *geo_title_screen(s32 sp50, struct GraphNode *sp54, void *context) {
    ...
    f32 scaleX = 1.0f;           // sp34
    f32 scaleY = 1.0f;           // sp30
    f32 scaleZ = 1.0f;           // sp2c
    ...
    if (sp50 != 1) {
        ...
        cameraMat = alloc_display_list(sizeof(*cameraMat));
        ...
        contextMtx = (f32 (*)[4]) context;
        if (intro_title_context_matrix_valid(contextMtx)) {
            mtxf_copy(titleCamera, contextMtx);
        } else {
            intro_title_make_default_camera(titleCamera);
        }

        mtxf_to_mtx(cameraMat, titleCamera);
        guScale(scaleMat, scaleX, scaleY, scaleZ);
        gSPMatrix(displayListIter++, cameraMat, G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
