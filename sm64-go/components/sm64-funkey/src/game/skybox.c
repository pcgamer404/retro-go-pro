...
static s32 sSkyboxMissingTextureLogCount;
...
void draw_skybox_tile_grid(Gfx **dlist, s8 background, s8 player, s8 colorIndex) {
    ...
    for (row = 0; row < 3; row++) {
        for (col = 0; col < 3; col++) {
            ...
            if (texture == NULL) {
                if (sSkyboxMissingTextureLogCount < 4) {
                    RG_LOGI("skybox_missing_tex[%d]: bg=%d player=%d tile=%d ul=%d",
                            (int) sSkyboxMissingTextureLogCount,
                            (int) background,
                            (int) player,
                            (int) tileIndex,
                            (int) sSkyBoxInfo[player].upperLeftTile);
                    sSkyboxMissingTextureLogCount++;
                }
                continue;
            }

            vertices = make_skybox_rect(tileIndex, colorIndex);
