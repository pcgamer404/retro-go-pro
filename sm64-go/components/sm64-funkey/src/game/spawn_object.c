 ...
 * an unimportant object if necessary.
 */
struct Object *allocate_object(struct ObjectNode *objList) {
    ...
    if (obj == NULL) {
        ...
        if (unimportantObj == NULL) {
            RG_LOGE("object pool exhausted (capacity=%d)", OBJECT_POOL_CAPACITY);
            RG_PANIC("SM64 object pool exhausted");
        } else {
            ...
            if (obj == NULL) {
                RG_LOGE("object pool allocation failed after freeing unimportant object");
                RG_PANIC("SM64 object allocation failed");
            }
