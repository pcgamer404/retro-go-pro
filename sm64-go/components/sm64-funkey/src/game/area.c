...
#else
            print_text_centered(160, 38, "PRESS");
            print_text_centered(160, 20, "START");
#endif
        }
    }
}
...
void load_obj_warp_nodes(void) {
    ...
    struct Object *startObj = (struct Object *) gObjParentGraphNode.children;
    struct Object *sp20 = startObj;
    s32 count = 0;
    ...
    do {
        ...
        sp20 = (struct Object *) sp20->header.gfx.node.next;
        if (++count > OBJECT_POOL_CAPACITY + 8) {
            RG_LOGE("load_obj_warp_nodes: object graph traversal exceeded capacity start=%p cur=%p next=%p",
                    startObj, sp1C, sp20);
            RG_PANIC("SM64 object graph loop");
        }
    } while (sp20 != startObj);
