...
#define OBJ_REL_TRANSFORM_DIAG_LIMIT 0

static s32 sObjRelTransformDiagCount;

static s32 obj_matrix_has_plausible_transform(Mat4 mtx) {
    f32 basis =
    ...
    return mtx[3][3] > 0.99f && mtx[3][3] < 1.01f && basis > 0.01f
...
void obj_build_transform_relative_to_parent(struct Object *obj) {
    ...
    s32 usedFallback = FALSE;

    obj_build_transform_from_pos_and_angle(obj, O_PARENT_RELATIVE_POS_INDEX, O_FACE_ANGLE_INDEX);
    obj_apply_scale_to_transform(obj);
    mtxf_mul(obj->transform, obj->transform, parent->transform);
    if (!obj_matrix_has_plausible_transform(obj->transform)) {
        obj->oPosX = parent->oPosX + obj->oParentRelativePosX;
        obj->oPosY = parent->oPosY + obj->oParentRelativePosY;
        obj->oPosZ = parent->oPosZ + obj->oParentRelativePosZ;
        obj_build_transform_from_pos_and_angle(obj, O_POS_INDEX, O_FACE_ANGLE_INDEX);
        obj_apply_scale_to_transform(obj);
        usedFallback = TRUE;
    } else {
    ...
    if (usedFallback && sObjRelTransformDiagCount < OBJ_REL_TRANSFORM_DIAG_LIMIT) {
        RG_LOGI("obj_rel_mtx_fb[%d]: flags=%04x rel=%d,%d,%d pos=%d,%d,%d parent=%d,%d,%d",
                (int) sObjRelTransformDiagCount,
                (unsigned int) obj->oFlags,
                (int) obj->oParentRelativePosX,
                (int) obj->oParentRelativePosY,
                (int) obj->oParentRelativePosZ,
                (int) obj->oPosX,
                (int) obj->oPosY,
                (int) obj->oPosZ,
                (int) parent->oPosX,
