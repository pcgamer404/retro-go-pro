...
static int state_store(race_state_t *rs)
{
  ...
  rs->fixsoundmahjong = fixsoundmahjong ? *fixsoundmahjong : 0;
  ...
  memcpy(rs->cpuram, cpuram, sizeof(rs->cpuram));
...
static int state_restore(race_state_t *rs)
{
  ...
  memcpy(cpuram, rs->cpuram, sizeof(rs->cpuram));
