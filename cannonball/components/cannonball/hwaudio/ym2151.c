...
/* ASG 980324 -- added for tracking timers */
    ...
    uint32_t   *tim_A_tab = NULL;     /* timer A deltas */
    uint32_t   *tim_B_tab = NULL;     /* timer B deltas */
#endif
...
*/
uint32_t       *freq = NULL;        /* 11 octaves, 768 'cents' per octave */
...
#else
                status &= ~1;
                //if ((status & 3)==1 && (irqhandler)) (*irqhandler)(device, 0);
#endif
            }
...
#else
                status &= ~2;
                //if ((status & 3)==2 && (irqhandler)) (*irqhandler)(device, 0);
#endif
            }
...
{
    if (!tim_A_tab) tim_A_tab = malloc(1024 * sizeof(uint32_t));
    if (!tim_B_tab) tim_B_tab = malloc(256 * sizeof(uint32_t));
    if (!freq)      freq      = malloc(11 * 768 * sizeof(uint32_t));

    YM_fps         = fps;
    YM_sample_freq = rate;
    YM_channels    = YM_STEREO;

    YM_frame_size =  YM_sample_freq / YM_fps;
    YM_buffer_size = YM_frame_size * YM_channels;
...
void YM_stream_update()
{
    ...
    for (i=0; i<length; i++)
    {
        ...
        YM_write_buffer(YM_LEFT,  i, (int16_t) (outl * YM_volume));
        YM_write_buffer(YM_RIGHT, i, (int16_t) (outr * YM_volume));
...
{
    uint32_t i;
    for (i = 0; i < YM_buffer_size; i++)
        YM_buffer[i] = 0;
}

void YM_write_buffer(const uint8_t channel, uint32_t address, int16_t value)
{
    //buffer[channel + (address * channels)] = (int16_t) (value * volume); // Unused for now
    YM_buffer[channel + (address * YM_channels)] = value;
}

int16_t YM_read_buffer(const uint8_t channel, uint32_t address)
{
    return YM_buffer[channel + (address * YM_channels)];
}

int16_t* YM_get_buffer()
{
    return YM_buffer;
}
