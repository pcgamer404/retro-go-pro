...
/* PICO-8 permits print shorthand in expression position, most notably
   `return ?value,x,y,color`.  The statement form is handled by shortprint()
   below; this form must retain the call result for the surrounding
   expression.  TK_EOL makes the shorthand argument list unambiguous. */
static void shortprintexp (LexState *ls, expdesc *v) {
  int line = ls->linenumber;
  FuncState *fs = ls->fs;
  expdesc args;

  static const char print_name[] = {'p', 'r', 'i', 'n', 't', 0};
  TString *n = luaS_new(ls->L, print_name);
  ls->t.seminfo.ts = n;
  ls->t.token = TK_NAME;
  singlevar(ls, v);
  ...
  if (!testnext(ls, TK_EOS)) {
    check(ls, TK_EOL);
    /*
    ** A short IF/WHILE owns the EOL that terminates its body.  Consuming it
    ** here made a print-shorthand short IF absorb every following statement
    ** up to the next ELSE/END. Standalone print shorthand still owns and
    ** consumes its own EOL.
    */
    if (ls->shortblockdepth == 0)
      luaX_next(ls);
  }
...
static void whilestat (LexState *ls, int line) {
  ...
  if (!short_while)
    ...
    /* Leave a shared EOL for an enclosing short block to consume. */
    if (ls->t.token == TK_EOS || ls->shortblockdepth == 1)
      luaX_next(ls);
  }
...
static void fornum (LexState *ls, TString *varname, int line) {
  ...
  if (testnext(ls, ','))
    ...
    luaK_codek(fs, fs->freereg, luaK_numberK(fs, (int32_t)1));
    luaK_reserveregs(fs, 1);
  }
