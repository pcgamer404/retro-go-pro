...
// Lua 5.2 reference code (llex) intentionally uses switch fall-through for
// decimal number parsing (`0` followed by `./x/X/b/B/digit`). PICO-8 Lua
// compiles with -Werror=implicit-fallthrough; silence just for this TU.
...
#define next(ls) (++(ls)->columnnumber, (ls)->current = zgetc((ls)->z))
...
static void read_numeral (LexState *ls, SemInfo *seminfo) {
  ...
  for (;;) {
    /* PICO-8's fixed-point literals do not use Lua's decimal/hex exponent
       syntax. This also permits token-saving adjacency such as `.5e8()`:
       number .5 followed by a call to function e8(). */
...
static int llex (LexState *ls, SemInfo *seminfo) {
  ...
  for (;;) {
    ...
    switch (ls->current) {
      ...
      case '?': {  /* PICO-8 print shorthand: statement or expression */
        next(ls);
